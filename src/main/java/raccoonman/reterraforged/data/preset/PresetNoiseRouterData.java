...
public class PresetNoiseRouterData {
    ...
    public static void bootstrap(Preset preset, BootstrapContext<DensityFunction> ctx) {
        ...
        // 1. Manually register and capture the Holder
        var offsetHolder = ctx.register(NoiseRouterData.OFFSET, DensityFunctions.add(
                DensityFunctions.constant(NoiseRouterData.GLOBAL_OFFSET - 0.5F),
                DensityFunctions.mul(
                        RTFDensityFunctions.clampToNearestUnit(RTFDensityFunctions.cell(CellField.HEIGHT), properties.terrainScaler()),
                        DensityFunctions.constant(2.0D)
                )
        ));

        // 2. Wrap the holder so it can be used as a DensityFunction variable
        DensityFunction offset = new DensityFunctions.HolderHolder(offsetHolder);

        // Now 'offset' can be used here just like before
