...
public abstract class PresetEditorPage extends BisectedPage<PresetConfigScreen, AbstractWidget, AbstractWidget> {
	...
	public class Preview extends Button {
		...
		public Preview() {
			// 1.21.1 Widget constructors typically use (x, y, width, height, message, onPress, narration)
			super(-1, -1, -1, -1, CommonComponents.EMPTY, (b) -> {
				Minecraft mc = Minecraft.getInstance();
				// MouseHandler is still accessible via the mc instance
				MouseHandler mouse = mc.mouseHandler;

				if (b instanceof Preview self) {
					// Use the mc instance to get the current window/scaling factor if needed for coordinates
					if (self.updateLegend((int) mouse.xpos(), (int) mouse.ypos()) && !self.hoveredCoords.isEmpty()) {
						self.playDownSound(mc.getSoundManager());

						// NEW PARADIGM: Access keyboardHandler via the mc instance directly
						// rather than tunneling through PresetEditorPage.this.screen.minecraft
						mc.keyboardHandler.setClipboard(self.hoveredCoords);
					}
				}
			}, DEFAULT_NARRATION);
		}
		...
		@Override
		// Changed from render to renderWidget
		protected void renderWidget(GuiGraphics guiGraphics, int mx, int my, float partialTicks) {
			int x = this.getX();
			int y = this.getY();

			// Keep your logic for square aspect ratio
			this.height = this.getWidth();

			RenderSystem.enableBlend();
			RenderSystem.defaultBlendFunc(); // Cleaner shortcut for the blendFuncSeparate calls

			// Rendering the preview texture
		...
		private float getLegendScale() {
			// Access options via the global Minecraft instance to avoid the protected 'screen.minecraft' field
			int guiScale = net.minecraft.client.Minecraft.getInstance().options.guiScale().get();

			// In 1.21.1, guiScale 0 is "Auto".
			// We adjust the index logic to handle the 0 (Auto) case explicitly if needed,
			// or keep your existing clamp logic which maps 0 or out-of-bounds to your fallback.
			int index = guiScale - 1;

			if (index < 0 || index >= LEGEND_SCALES.length) {
				// Handle Auto (0) or unusually high scales from mods
